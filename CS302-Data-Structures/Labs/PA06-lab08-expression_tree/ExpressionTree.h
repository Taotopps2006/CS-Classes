...
class ExprTree {
  ...
  private:
	...
	void copy(ExprTreeNode*& current, ExprTreeNode* source);
	void buildHelper(ExprTreeNode*& node);
	void expressionHelper(const ExprTreeNode* p) const;
	DataType evalHelper(const ExprTreeNode* p) const;
	void clearHelper(ExprTreeNode*& p);
	void commuteHelper(ExprTreeNode*& p);
	bool isEquivalentHelper(ExprTreeNode* p, ExprTreeNode* other) const;
