...
{
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": start processing action");
	int sleepTime;
	...
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": end processing action");
}

void ProcessControlBlock::newInputThread(string operation, int cycleTime)
{
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": start " + operation + " input");
	int sleepTime;
	...
	thread process(createThreadThatSleeps, sleepTime);
	process.join(); // Pause until thread is done
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": end " + operation + " input");
}

void ProcessControlBlock::newOutputThread(string operation, int cycleTime)
{
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": start " + operation + " output");
	int sleepTime;
	...
	thread process(createThreadThatSleeps, sleepTime);
	process.join(); // Pause until thread is done
	myLog.logProcess(string("Process ") + to_string(processNumber) + ": end " + operation + " output");
}